...
# Blogger + Instagram + Threads ship every day regardless.
...
if [ "$NAVER_SCHEDULE" = "daily" ]; then
    NAVER_MODE="Today is on the NAVER-DAILY schedule (naver_schedule=daily): publish directly to Naver as well, every day — same as Blogger, Instagram, and Threads."
else
  DOW=$(date +%u) # 1=Mon .. 7=Sun
  if [ "$DOW" = "1" ] || [ "$DOW" = "3" ] || [ "$DOW" = "5" ]; then
    NAVER_MODE="Today IS a Naver day (Mon/Wed/Fri): publish directly to Naver as well."
  else
    NAVER_MODE="Today is NOT a Naver day: SKIP Naver entirely (no draft, no publish) — ship Blogger + Instagram + Threads only. The topic's Naver version ships on the next Mon/Wed/Fri run only if still fresh; otherwise it lives on Blogger/IG/Threads alone."
  fi
fi

claude -p "Invoke the ship-topic skill and run the full AUSSIE UMMA pipeline end to end for today's date. This is a PRE-APPROVED unattended daily scheduled run set up by the user: SKIP the Stage 3 review gate, do NOT ask for confirmation. $NAVER_MODE Publish Instagram via the GitHub workflow with both date and slug so the dependent Threads workflow publishes the validated threads/thread.json artifact. Generate blog images with 'node scripts/gen-images.mjs --date <today>' (headless via .gemini-profile), then re-run scripts/md-to-blogger.mjs so blogger.html picks them up. Publish Blogger with 'node scripts/post-to-blogger.mjs --date <today>' (headless via .blogger-profile). Do NOT use claude-in-chrome for any of this — it is not available in a scheduled session. Always debug Naver fixes in --draft; never re-run the direct publish to test." \
