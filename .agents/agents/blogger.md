...
3. `.agents/rules/brand-rules.md` — Blog-specific rules
...
7. **마무리** — one paragraph, empathetic close
   ...
   Write the hashtags as plain lines only. Never add a `## 태그` heading because
   Naver turns H2 headings into numbered quotation blocks.
...
Start the file directly with the public Markdown H1 title. Never add hidden or
visible metadata headers, YAML front matter, or HTML comments such as
`<!-- title: ... -->`. Search queries and target keywords are workflow-only
brief data and must not appear in `post.md` unless naturally used in the public
article body. Keep image prompts only in the public workflow section described
above; publishing converters remove that section.
